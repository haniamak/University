class Wyrazenie:
  def oblicz(self, zmienne):
    raise NotImplementedError("Metoda oblicz() nie została zaimplementowana w podklasach.") 

  def __str__(self):
    raise NotImplementedError("Metoda __str__() nie została zaimplementowana w podklasach.")
  
  def pochodna(self, zmienna):
    raise NotImplementedError("Metoda pochodna() nie została zaimplementowana w podklasach.") 
...
class Zmienna(Wyrazenie):
  ...
  def oblicz(self, zmienne):
    if self.nazwa not in zmienne:
      raise VariableNotFoundException(f"Zmienna {self.nazwa} nie została znaleziona w słowniku")
...
class Razy(Wyrazenie):
  ...
  def pochodna(self, zmienna):
    ...
    return Dodaj(Razy(self.w1.pochodna(zmienna), self.w2), Razy(self.w1, self.w2.pochodna(zmienna)))
...
class Podziel(Wyrazenie):
  ...
  def pochodna(self, zmienna):
    ...
    licznik = Odejmij(Razy(self.w1.pochodna(zmienna), self.w2), Razy(self.w1, self.w2.pochodna(zmienna)))
