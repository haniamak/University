...
def rozwiaz_sudoku_wszystkie(s, rozwiazania):
    pustepole = znajdzpustepole(s)
    
    if not pustepole:
        # Dodaj kopię rozwiązania do listy rozwiązań
        rozwiazania.append([row[:] for row in s])
        return
    ...
    for num in range(1, 10):
        if czywpole(s, row, col, num):
            ...
            rozwiaz_sudoku_wszystkie(s, rozwiazania)
...
rozwiaz_sudoku_wszystkie(sudoku, rozwiazania)

print("Znaleziono", len(rozwiazania), "rozwiązań.")
i = 1
for rozwiazanie in rozwiazania:
